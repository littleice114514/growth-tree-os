...
| Profile | 叶子 | 果实 | 持续动画 | DPR | 默认定位 |
| --- | ---: | ---: | ---: | ---: | --- |
| low | 80-150 | 20 以内 | 10 以内 | 1.0 | MacBook Air / 无独显 / 低功耗 |
| medium | 200-500 | 50 以内 | 30 以内 | 1.0-1.5 | 默认稳定档 |
| high | 800-1500 | 100 以内 | 80 以内 | 1.5-2.0 | Windows 独显开发和演示 |

不允许 high 作为默认档位。Medium 是默认稳定档，low 是保底可用档。
...
- 不允许单个大模型阻塞整棵树渲染。
- 叶子未来优先使用 instancing。
- 文字标签不能全部常驻显示，只在 hover、选中、搜索或摘要模式出现。
- 大贴图不能默认加载到 Mac。
...
3D 不允许成为整个系统的单点故障。即使 3D 失败，复盘、时间负债、财富、健康、项目记录和 Life Tree 快照仍必须可读、可保存、可验收。
