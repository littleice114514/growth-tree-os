...
- 展开态提供时间控制台，包括当前状态、任务名称输入、一级分类选择、开始计时、收起操作。
- 一级分类第一阶段固定选项：工作 / 学习 / 休息 / 生活 / 其他，默认 `学习`。
- 计时中显示当前计时卡片，包括任务名、一级分类、开始时间、已用时和结束计时。
- 结束计时后显示最近一次记录反馈。
- 从已有 Time Debt logs 中提取最多 3 个最近任务名，支持快捷填入任务名和一级分类。
...
- 小控制台不遮挡关键区域
- 任务选择/输入正常
- 一级分类选择可见，选项包含工作 / 学习 / 休息 / 生活 / 其他
- 计时中卡片显示一级分类
- 结束记录后 Time Debt 可见
- 最近记录反馈可见
- 最近任务快捷填入在有历史 logs 时可用，并能回填一级分类
