  ...
  return (
    <section className="rounded-[18px] border border-[color:var(--panel-border)] bg-[var(--panel-bg-strong)] p-4">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          <div className="text-[10px] uppercase tracking-[0.18em] text-[color:var(--text-muted)]">Insight</div>
          <h3 className="mt-1 text-base font-semibold text-[color:var(--text-primary)]">开销去向</h3>
        </div>
      ...
      {breakdown.slices.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-[color:var(--panel-border)] p-6 text-center">
          <div className="text-sm font-medium text-[color:var(--text-secondary)]">这段时间还没有支出记录</div>
          <p className="mt-2 text-xs text-[color:var(--text-muted)]">
            记录一笔真实支出后，系统会分析钱花到哪里去了。
          </p>
        </div>
      ) : (
        <>
            ...
            {breakdown.slices.map((s, i) => (
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  />
                  <span className="text-xs font-medium text-[color:var(--text-primary)]">{s.category}</span>
                </div>
          ...
          <div className="mt-3 rounded-xl border border-[color:var(--panel-border)] bg-[var(--inspector-section-bg)] px-3 py-2 text-right">
            ...
            <span className="text-xs font-semibold text-[color:var(--text-primary)]">{formatCurrency(breakdown.total)}</span>
          </div>
        </>
      )}
    </section>
